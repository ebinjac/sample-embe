...
export default function TeamLinksPage({ params }: { params: { teamId: string } }) {
  ...
  // Fetch team links (public links)
  const fetchTeamLinks = async (filters = {}) => {
    if (!userId) return
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      if (searchQuery !== undefined) {
        fetchTeamLinks(searchQuery ? { search: searchQuery } : {})
      }
    }, 300)
  ...
  const handleLinkUpdated = (updatedLink: Link) => {
    if (updatedLink.visibility === 'public') {
      setLinks(prev => prev.map(link => 
        link.id === updatedLink.id ? updatedLink : link
      ))
    } else {
      // If link was made private, remove it from the list
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-3xl font-bold">Team Links</h1>
            <p className="text-muted-foreground">
              Links visible to everyone in the team
            </p>
          </div>
        </div>
      ...
      {showFilters && (
        <SearchFilter
          onFilterChange={(filters: any) => fetchTeamLinks(filters)}
          teamId={teamId}
        />
      )}
      ...
      {loading ? (
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            <p className="text-muted-foreground">Loading team links...</p>
          </div>
        </div>
      ) : error ? (
        <Card>
          <CardContent className="pt-6">
            <div className="text-center text-destructive">
              <p>{error}</p>
              <Button variant="outline" className="mt-2" onClick={() => fetchTeamLinks()}>
                Try Again
              </Button>
            </div>
          </CardContent>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-center">
              ...
              <h3 className="text-lg font-medium mb-2">No team links</h3>
              <p className="text-muted-foreground mb-4">
                {searchQuery ? 'Try adjusting your search terms' : 'Create public links that everyone in the team can see'}
              </p>
              {!searchQuery && (
                <Button onClick={() => setShowCreateForm(true)}>
                  <Plus className="h-4 w-4 mr-2" />
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              onClick={() => fetchTeamLinks({ page: pagination.page - 1 })}
            >
              Previous
            </Button>
