...
export default function PinnedPage({ params }: { params: { teamId: string } }) {
  ...
  // Fetch pinned links
  const fetchPinnedLinks = async (filters: { search?: string } = {}) => {
    if (!userId) return
    ...
    try {
      ...
      let filteredLinks = result
      if (filters.search) {
        const searchLower = filters.search.toLowerCase()
        filteredLinks = result.filter(link =>
  ...
  useEffect(() => {
    const timeoutId = setTimeout(() => {
      if (searchQuery !== undefined) {
        fetchPinnedLinks(searchQuery ? { search: searchQuery } : {})
      }
    }, 300)
  ...
  const handleLinkUpdated = (updatedLink: Link) => {
    if (updatedLink.isPinned) {
      setLinks(prev => prev.map(link => 
        link.id === updatedLink.id ? updatedLink : link
      ))
    } else {
      // If link was unpinned, remove it from the list
    ...
    <div className="container mx-auto p-6 space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-2">
          ...
          <div>
            <h1 className="text-3xl font-bold">Pinned Links</h1>
            <p className="text-muted-foreground">
              Quick access to your important links
            </p>
          </div>
        </div>
      ...
      {showFilters && (
        <SearchFilter
          onFilterChange={(filters: any) => fetchPinnedLinks(filters)}
          teamId={teamId}
        />
      )}
      ...
      {loading ? (
        <div className="flex items-center justify-center h-64">
          <div className="text-center">
            ...
            <p className="text-muted-foreground">Loading pinned links...</p>
          </div>
        </div>
      ) : error ? (
        <Card>
          <CardContent className="pt-6">
            <div className="text-center text-destructive">
              <p>{error}</p>
              <Button variant="outline" className="mt-2" onClick={() => fetchPinnedLinks()}>
                Try Again
              </Button>
            </div>
          </CardContent>
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-center">
              <PinIcon className="h-12 w-12 mx-auto mb-4 text-muted-foreground" />
              <h3 className="text-lg font-medium mb-2">No pinned links</h3>
              <p className="text-muted-foreground mb-4">
                {searchQuery ? 'Try adjusting your search terms' : 'Pin important links for quick access'}
              </p>
              {!searchQuery && (
